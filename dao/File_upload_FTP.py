...
path = './'
FTP_path = path+'/FTP配置文件.xlsx'
upload_file_FTP_path = path+'/需上传的文件'
files = []
for file_name in os.listdir(upload_file_FTP_path):
    file = os.path.join(upload_file_FTP_path,file_name)
    files.append(file)
...
Valus = Read_configfile(FTP_path)
...
class MyFTP:
    ...
    def __init__(self, host, port=Valus[1]):
        """ 初始化 FTP 客户端
        参数:
                 host:ip地址
...
if __name__ == "__main__":
    ...
    my_ftp = MyFTP(Valus[0])

    my_ftp.login(Valus[2], Valus[3])
    ...
    #     # 上传单个文件
        ...
        my_ftp.upload_file(file, Valus[4]+'/'+ file_name)

    # 上传目录
    # my_ftp.upload_file_tree(r'D:\Work\Project\TestData', "/")
